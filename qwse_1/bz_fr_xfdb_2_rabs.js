function bz_xfdb_2(neig_kp) {
    ...
    const imfb_x = canvas.width / 2;
    const imfb_y = canvas.height
    let qhsu_x = imfb_x
    let qhsu_y = imfb_y//todo
    // 方块大小
    const size = 4;
    ...
    function animate() {
        ...
        // 计算时间相关的角度（让它们动起来）
        const angle1 = (Date.now() + neig.uj_1_zdti_nokz) * neig.uj_1_angle_tiqe * neig.uj_1_ldsc // 随着时间变化的角度

        const angle2 = (Date.now() + neig.uj_2_zdti_nokz) * neig.uj_2_angle_tiqe * neig.uj_2_ldsc // 随着时间变化的角度
        ...
        // 绘制第一个绿色方块
        ctx.fillStyle = '#2E7D32'//'#4CAF50'; // 绿色
