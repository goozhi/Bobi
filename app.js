...
// first page match
app.use(async (ctx, next) => {
    if (/Android [0-9]\b|iPhone OS (?:[0-9][^\d]|1[0-1][^\d])|^(?:(?!Windows|Mac|iPhone|Android|[Uu]buntu).)+$/.test(ctx.header['user-agent'])) {
        if (fs.existsSync(yxna_wrvr))
            fs.appendFile(path.join(yxna_wrvr, 'blackNameList.txt'), `${new Date().toString()} ${ctx.header['user-agent']}\n`, (err) => { })
