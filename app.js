...
app.use(async (ctx, next) => {
    if (ctx.path === '/rsgm') {
        const { yxna_rjqt, content, ji_ce_yxna, ji_rjqt, ji_rjqt_tum } = ctx.request.body
        if (yxna_rjqt) {
            if (content) {
                ...
                if (ji_ce_yxna || fs.existsSync(nixb_yxna)) {
                    if (fs.existsSync(nixb_yxna)) {
                        ji_rjqt && fs.writeFileSync(nixb_yxna + ".bak", fs.readFileSync(nixb_yxna))
                    }
                    ji_rjqt && fs.writeFileSync(nixb_yxna, bqeo)
                    if (ji_rjqt_tum && !fs.existsSync(nixb_yxna)) {
                        fs.mkdirSync(nixb_yxna, { recursive: true })
                    }
            ...
            .then(res => {
                ...
                prvd_afoa('x ytjp --kp \`${yo_zzuy_1.get_yoch_dyih()},${shn1.get_yoch_dyih()},'+vdzv_rj+'\`').then(res=>alert(res)).catch(e=>alert(e.message||e.reason||e))
                    ...
                    ">add</button>`
                        ...
                        //                 , get_ds_bqeo: (vxn, shn1, slm1) => "->" +`<button class="is-hidden afoa-ubqt" onClick="fetch('/afoa',{
                        //                 method:'POST'
...
jplp_rjqt.jplp('styles')
jplp_rjqt.jplp('zzzz', { nikc_kp: path.resolve('../zzzz') })
jplp_rjqt.jplp('scripts', { nikc_kp: path.resolve('../scripts') })
jplp_rjqt.jplp('wrvr_imgs', { nikc_kp: path.resolve('../wrvr_imgs') })
jplp_rjqt.jplp('sdcard', { nikc_kp: path.resolve('/sdcard/') })
jplp_rjqt.jplp('storage', { nikc_kp: path.resolve('/storage/') })
jplp_rjqt.jplp('D', { nikc_kp: path.resolve('D:/') })
